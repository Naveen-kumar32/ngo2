...
export function Shell({children}){
 ...
 return <><motion.div className="progress" style={{scaleX:scale}}/><header className="header"><Link className="logo" to="/"><span className="logo-mark">INF</span><span>INDIAN NATIONAL<span> NGO FEDERATION</span></span></Link><nav className={open?'nav open':'nav'}>{nav.map(([n,p])=><NavLink key={p} to={p}>{n}</NavLink>)}<Link className="nav-cta" to="/register">Join network <ArrowUpRight size={16}/></Link></nav><button className="menu" onClick={()=>setOpen(!open)} aria-label="menu">{open?<X/>:<Menu/>}</button></header>{children}<footer className="footer"><div><Link className="logo light" to="/"><span className="logo-mark">INF</span><span>INDIAN NATIONAL<span> NGO FEDERATION</span></span></Link><p>A modern network connecting people, organisations and resources to measurable community impact across India.</p></div><div className="footer-links"><div><b>Explore</b><Link to="/causes">Causes</Link><Link to="/ngos">NGOs</Link><Link to="/projects">Projects</Link><Link to="/stories">Stories</Link></div><div><b>Participate</b><Link to="/volunteer">Volunteer</Link><Link to="/csr">CSR Connect</Link><Link to="/register">Register NGO</Link><Link to="/resources">Resources</Link></div></div><div className="footer-bottom"><span>© 2026 ImpactGrid Network</span><span>Built for transparent, local impact.</span></div></footer></>
}
